pipeline {
   ...
   stages {
      ...
      //  bat label: '', script: ' C:/"Program Files (x86)"/IIS/"Microsoft Web Deploy V3"/msdeploy.exe -verb:sync -source:package="Publish.zip" -dest:auto -setParam:name="Publish",value="Default Web Site/Publish"'
         bat label: '', script: ' C:/"Program Files (x86)"/IIS/"Microsoft Web Deploy V3"/msdeploy.exe -verb:sync -source:package="Publish.zip" -dest:auto,computerName="http://172.31.81.195/msdeploy.axd?site=my_iis_site",username=Administrator,password=pW.oJ2icenll75J&hJF$=PuUnK6M7RN-,authType=basic -allowUntrusted=true'
